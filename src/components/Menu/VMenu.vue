    ...
    <Menu>
      <MenuButton class="mb-1">
        <slot>
          ...
          @click="show = !show">
            <button class="leading-[0px] pb-[15px] pt-1 px-1">...</button>
          </div>
        </slot>

      </MenuButton>
      <MenuItems
      class="flex flex-col absolute w-fit py-2 px-4 bg-white border border-primary-500 rounded-md shadow-md space-y-2 items-group">
        <slot name="menuItems" />
      </MenuItems>
  </Menu>
</template>
